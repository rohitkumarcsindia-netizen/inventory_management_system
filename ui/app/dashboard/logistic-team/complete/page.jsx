...
const normalizeFinanceRecord = (rec) => {
  ...
  let approved = rec.actionBy || o.actionBy;
  ...
  return {
    ...
    pdiAction: rec.pdiAction || o.pdiAction,
    actionTime: rec.actionTime || rec.actionTime || o.actionTime,
    logisticsComment: rec.logisticsComment || o.logisticsComment,
    approvedByUserId: approvedUserId,
    actionBy: rec.actionBy || o.actionBy

  };
};

export default function AmispCompleteOrders() {
...
// FETCH ORIGINAL DATA
  const fetchOrders = async () => {
    try {
      ...
      const data = await httpService.get(
        `/api/v1/orders/logistic/complete?offset=${offset}&limit=${ordersPerPage}`
      );
      ...
      console.log("Amisp Fetch Error:", err);
    }
  };
  ...
  const applyDateFilter = async () => {
    ...
    const data = await httpService.get(
      `/api/v1/orders/logistic/complete/date-filter?startDate=${startDate}&endDate=${endDate}&page=${page}&size=${ordersPerPage}`
    );
  ...
  const applyStatusFilter = async (value) => {
    ...
    const data = await httpService.get(
      `/api/v1/orders/logistic/complete/status-filter?status=${value}&page=${page}&size=${ordersPerPage}`
    );
  ...
  const applySearchFilter = async (text) => {
    ...
    const page = currentPage - 1;
    const data = await httpService.get(
      `/api/v1/orders/logistic/complete/search?keyword=${text}&page=${page}&size=${ordersPerPage}`
    );
