...
// FETCH ORIGINAL DATA
  const fetchOrders = async () => {
    try {
      ...
      const data = await httpService.get(
        `/api/orders/logistic/complete?offset=${offset}&limit=${ordersPerPage}`
      );
  ...
  const applyDateFilter = async () => {
    ...
    const data = await httpService.get(
      `/api/orders/logistic/complete/date-filter?startDate=${startDate}&endDate=${endDate}&page=${page}&size=${ordersPerPage}`
    );
  ...
  const applyStatusFilter = async (value) => {
    ...
    const data = await httpService.get(
      `/api/orders/logistic/complete/status-filter?status=${value}&page=${page}&size=${ordersPerPage}`
    );
  ...
  const applySearchFilter = async (text) => {
    ...
    const page = currentPage - 1;
    const data = await httpService.get(
      `/api/orders/logistic/complete/search?keyword=${text}&page=${page}&size=${ordersPerPage}`
    );
