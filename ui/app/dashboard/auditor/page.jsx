...
const normalizeFinanceRecord = (rec) => {
  ...
  return {
    ...
    orderType: rec.orderType || o.orderType,
    status: rec.status || o.status,
    reasonForBuildRequest: rec.reasonForBuildRequest || o.reasonForBuildRequest,
...
export default function AuditorOrders() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const data = await httpService.get(
        `/api/v1/orders/auditor/all-orders?offset=${offset}&limit=${ordersPerPage}`
      );
  ...
  const applyStatusFilter = async (value) => {
    ...
    const data = await httpService.get(
      `/api/v1/orders/auditor/status-filter?status=${value}&page=${page}&size=${ordersPerPage}`
    );
  ...
  const applyDateFilter = async () => {
    ...
    const page = currentPage - 1;
    const data = await httpService.get(
      `/api/v1/orders/auditor/date-filter?startDate=${startDate}&endDate=${endDate}&page=${page}&size=${ordersPerPage}`
    );
  ...
  const applySearchFilter = async (text) => {
    ...
    const page = currentPage - 1;
    const data = await httpService.get(
      `/api/v1/orders/auditor/search?keyword=${text}&page=${page}&size=${ordersPerPage}`
    );
  ...
  return (
    <div className="min-h-screen w-full bg-[#e3f3ff] flex flex-col items-center py-10 relative">
      ...
      <div className="flex items-center gap-3 mb-4 mt-5">
        ...
        <h1 className="text-4xl font-bold text-[#02A3EE]">ORDER TRACKER</h1>
      </div>
